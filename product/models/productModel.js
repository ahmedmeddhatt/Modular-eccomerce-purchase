...
const getAllProducts = async (dbConnection) => {
    const result = await 
    dbConnection.query(`SELECT id, name, description, price FROM products`);
    return result.rows;
};

const getProductById = async (id, dbConnection) => {
    const result = await dbConnection.query(`SELECT id, name, description, price FROM products 
    WHERE id = $1`, [id]);
...
const deleteProduct = async (id, dbConnection) => {
    const result = await dbConnection.query(`DELETE FROM products WHERE id = $1 RETURNING *`, [id]);
    return result.rows[0];
};
